...
if %errorlevel% neq 0 (
    ...
    echo.
    set /p INSTALL_GIT="  다운로드 페이지를 열까요? (Y/N): "
    if /i "!INSTALL_GIT!"=="Y" (
...
if %errorlevel% neq 0 (
    ...
    echo.
    set /p INSTALL_NODE="  다운로드 페이지를 열까요? (Y/N): "
    if /i "!INSTALL_NODE!"=="Y" (
...
if !errorlevel! neq 0 (
    ...
    echo   [OK] 최신 코드 (브랜치: !CURRENT_BRANCH!)
)
echo.
...
if not exist ".env" (
    ...
    echo      https://developers.naver.com/apps 에서 앱 등록
    echo      사용 API: "검색" 선택 후 Client ID / Secret 발급
    echo.

    set /p ANT_KEY="  Anthropic API Key (sk-ant-...): "
    if "!ANT_KEY!"=="" (
        echo   [!] API Key 미입력. .env.example을 .env로 복사합니다.
        echo   .env 파일을 직접 편집하세요.
...
if exist "!PW_DIR!" (
    for /d %%d in ("!PW_DIR!\chromium-*") do (
        if exist "%%d\chrome-win\chrome.exe" set CHROMIUM_OK=1
    )
)

:: 시스템 Chrome 확인
if !CHROMIUM_OK! equ 0 (
    if exist "%PROGRAMFILES%\Google\Chrome\Application\chrome.exe" set CHROMIUM_OK=1
    if exist "%LOCALAPPDATA%\Google\Chrome\Application\chrome.exe" set CHROMIUM_OK=1
)

if !CHROMIUM_OK! equ 0 (
    echo   Chromium 설치 중 (더벨 기사 추출에 필요)...
